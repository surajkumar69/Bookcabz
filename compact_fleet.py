...
pattern = r'(<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-).*?(">).*?(</div>\s*</div>\s*</section>)'
match = re.search(pattern, html, flags=re.DOTALL)

if match:
    # Change grid gap from 8 to 5 for a tighter desktop layout
    grid_start = match.group(1) + '5' + match.group(2)
    new_html = html[:match.start(1)] + grid_start + '\\n' + cards_html + '\\n            ' + match.group(3) + html[match.end(3):]
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(new_html)
    print("Successfully compacted fleet section")
else:
    print("Could not find the fleet grid container")
